 ...
 const archivedItems = document.querySelector(".archivedItems")
 const archivedItemsBox = document.querySelector(".archivedItemsBox")
 const archivedNrEl = document.getElementById("archived-nr")
 const archiveTitle = document.getElementById("archive-title")
 const archiveTitleBox = document.querySelector(".archiveTitleBox")
 const clearArchiveBtn = document.getElementById("clear-archive-button")
 let fontSizeBar = ""
 let archiveOpen = "off"
 ...
 onValue (ref(db,"archive/"), (snapshot) => {
    ...
    let archiveArray = Object.entries (snapshot.val())
    let archivedNr = 0

    for (let i = 0; i < archiveArray.length; i++) {
        
        let archivedItem = document.createElement("li")
        let archivedItemId = archiveArray[i][0]
        archivedItem.setAttribute("class","archivedItem")
        archivedItem.textContent = archiveArray[i][1]
...
fontSize.addEventListener("click", () => {
 ...
 archiveTitle.addEventListener("click", () => {
    if ((archiveOpen == "off") && (archivedNrEl.textContent != "0")){
    archiveOpen = "on"
    archivedItemsBox.style.height = "fit-content"
...
clearArchiveBtn.addEventListener("click", () => {
    remove(ref(db,"archive/"))
    archivedItems.innerHTML = ""
    archivedItems.style.margin = "0px"
    archivedNrEl.textContent = "0"
